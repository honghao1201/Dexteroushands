...
try:
    ...
    from rl.pregrasp_env import AeroPregraspEnv
...
def collect_rollout(model_path: pathlib.Path, seed: int):
    ...
    base = DummyVecEnv([lambda: AeroPregraspEnv()])
...
class ReplayWindow:
    ...
    def __init__(self, sim, frames, fps: int = 20):
        ...
        self.root = tk.Tk()
        self.root.title("Pregrasp 阶段三视图进度回放")
    ...
    def render_current(self):
        ...
        self.status.configure(text=(
            ...
            f"平均间隙误差: {float(info.get('mean_gap_error', float('nan'))):.4f} m    "
            f"两侧关系: {info.get('opposite_side', False)}    "
...
def main():
    ...
    parser.add_argument("--model", type=pathlib.Path, default=pathlib.Path("rl/checkpoints/aero_pregrasp_ppo_v4"))
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--fps", type=int, default=20)
    args = parser.parse_args()
    model_path = resolve_model_path(args.model)
    sim, frames, info = collect_rollout(model_path, args.seed)
    print(f"已记录 {len(frames)} 帧，最终平均间隙误差 {info.get('mean_gap_error', float('nan')):.4f} m，成功={info.get('success', False)}")
