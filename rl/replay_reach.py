...
try:
    ...
    from rl.reach_env import AeroReachEnv
...
def collect_rollout(model_path: pathlib.Path, seed: int):
    ...
    base = DummyVecEnv([lambda: AeroReachEnv()])
...
class ReplayWindow:
    ...
    def __init__(self, sim, frames, fps: int = 20):
        ...
        self.root = tk.Tk()
        self.root.title("Reach 阶段三视图进度回放")
    ...
    def render_current(self):
        ...
        self.status.configure(text=(
            ...
            f"Reach成功: {info.get('success', False)}"
        ))
...
def main():
    ...
    parser.add_argument("--model", type=pathlib.Path, default=pathlib.Path("rl/checkpoints/aero_reach_ppo_v1"))
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--fps", type=int, default=20)
    args = parser.parse_args()
    model_path = resolve_model_path(args.model)
    sim, frames, info = collect_rollout(model_path, args.seed)
    print(f"已记录 {len(frames)} 帧，最终误差 {info.get('anchor_error', float('nan')):.4f} m，成功={info.get('success', False)}")
